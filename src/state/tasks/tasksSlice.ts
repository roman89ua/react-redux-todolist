...
const tasksSlice = createSlice({
  ...
  reducers: {
    ...
    //   const elementIndex = findIndexByIndex(state.tasks, action.payload.id)
    //   state.tasks.splice(elementIndex, 1, action.payload)
    // },
    ...
    statusUpdate: (state, action: PayloadAction<{ id: string }>) => {
      ...
      state.tasks.splice(elementIndex, 1, {
        ...
        status:
          state.tasks[elementIndex].status === TasksFilters.notCompleted
            ? TasksFilters.completed
            : TasksFilters.notCompleted,
      })
    },
...
export const { addTask, deleteTask, filterChange, statusUpdate } = tasksSlice.actions
